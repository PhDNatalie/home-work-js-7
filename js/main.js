...
function replaceHTMLComments(str2) {
  return str2.replace(/<!--(?!>)[\S\s]*?-->\s*/g, '').trim();
};
...
let validateFileType = (str3) => {
  return (/(\.jpg|\.jpeg|\.png)$/).test(str3);
};
...
let checkYear = (str4) => {
  return (/^(19\d{2}|20\d{2}|2100)$/).test(str4);
};
...
let checkEmail = (str5) => {
  return (/^[a-zA-Z-.]+@[a-z\.]+\.[a-z]{2,3}$/).test(str5);
};
...
let checkDomainUrl = (str6) => {
  return (/^https?:\/\/(?!:\/\/)([-_\w]+\.)*[\w][-_\w]+\.[a-zA-Z]{2,5}?$/i).test(str6);
};
...
let createLinksFromDomains = (str7) => {
  let reg = new RegExp('(https?:)\\/\\/(?!:\\/\\/)([-_\\w]+\\.)*[\\w][-_\\w]+\\.[\\w]{2,5}', 'gi');

  str7.match(reg).forEach((item) => str7 = str7.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(https?:)\/\//gi, '')}</a>`));
